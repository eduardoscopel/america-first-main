import { getManagerRecords, round } from '$lib/utils/helper';

let recordManID = 1;

export const getTable = async (recordManID) => {
				...
				if(!playerPositionData[player.playerInfo.pos].players[player.playerID]) {
					if(!player.playerInfo.pos || player.playerInfo.pos == null) {
						...
						const entry = {
							...
							fptspg: managerRecords.playerBook.totals.alltime[recordManID].combined.find(p => p.playerID == player.playerID).playerPPStart,
							weeks: [],
							id: 'player',
						}
				...
				for(const player in playerPositionData[position].players) {
					if(playerPositionData[position].players[player].started == true) {
						...
						const entry = {
							...
							avatar: playerPositionData[position].players[player].avatar,
							position: playerPositionData[position].players[player].position,
							weeks: playerPositionData[position].players[player].weeks,
							id: 'parent',
						}
